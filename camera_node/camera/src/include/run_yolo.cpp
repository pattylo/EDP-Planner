...
void run_yolo::findwhichboundingboxrocks(vector<cv::Mat> &netOutput, cv::Mat &frame)
{
            ...
            //if there is, further extract the data
            {
                ...
                for(int class_i=0;class_i<classnames.size();class_i++)//as for this step, this for loop take the probabilities of every class
                {
                    auto confidence_each_class = output.at<float>(i, 5+class_i); //6th element will be the 1st class confidence, class id=0
                        //7th element will be the 2nd class confidence, class id=1
    ...
    for(int i =0 ; i < indices.size();i++)
    {
        ...
        float detectedconfidence = confidenceperbbox[index];
        string textoutputonframe = detectedclass + ": " + to_string(detectedconfidence);
        ...
        if(index%4 == 0)
        {
            color = colorforboxes[0];
        }
        if(index%4 == 1)
        {
            color = colorforboxes[1];
        }
        if(index%4 == 2)
        {
            color = colorforboxes[2];
        }
        if(index%4 == 3)
        {
            color = colorforboxes[3];
        }
        cv::rectangle(frame, cv::Point(final_x, final_y), cv::Point(final_x+final_w, final_y+final_h), color,2);
        cv::putText(frame, textoutputonframe, cv::Point(final_x,final_y-10),cv::FONT_HERSHEY_COMPLEX_SMALL,1,cv::Scalar(200,200,200));
