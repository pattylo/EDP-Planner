...
void circle_heading(double &dg, double &rd, double &rd_end, double center_x, double center_y, geometry_msgs::PoseStamped& pose_local, int i, int &fly_mode, ros::Time &last_request_local, ros::Time instant_time, double loop, bool clockwise);
...
//process~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      {
         movement(end_x, end_y, end_z, 0, 0, 2, pose, j, fly,last_request, ros::Time::now());
         j++;
      }
      if( ros::Time::now() - last_request > ros::Duration(2.0) && fly == 2)
      {
         movement(end_x, end_y, end_z, 2, 2,  2, pose, k, fly,last_request, ros::Time::now());
         k++;
      }
      if( ros::Time::now() - last_request > ros::Duration(2.0) && fly == 3)
      {
        cout<<"now change heading"<<endl;
        rpy_to_Q(dg,w,x,y,z);
      ...
      {
        circle_heading(degree, radius, end_radius, 1, 2, pose,m,fly, last_request, ros::Time::now(), 1, clockwise);
        m++;
      }
      if( ros::Time::now() - last_request > ros::Duration(2.0) && fly == 5)
      {
        movement(end_x,end_y,end_z, 0,0,2,pose, i, fly, last_request,ros::Time::now());
        i++;
      }
      ...
      if( ros::Time::now() - last_request > ros::Duration(2.0) && fly == 3)
      {
        pose.pose.orientation.w = sqrt(2)/2;
        pose.pose.orientation.x = 0;
        pose.pose.orientation.y = 0;
        pose.pose.orientation.z = sqrt(2)/2;
      }*/
...
//landing~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if (uavx-end_x < 0.1 && uavx-end_x >-0.1 && uavy-end_y < 0.1 && uavy-end_y >-0.1 && ros::Time::now() -last_request>ros::Duration (2.0) && fly==6)
    {
        cout<<"UAV now preparing to land...." <<endl;
        fly++;//landing parameter
        last_request=ros::Time::now();
    }
    if(ros::Time::now()-last_request >ros::Duration(2.0) && fly==7)
    {
...
//need mofification on the speed
...
void circle_heading(double &dg, double &rd, double &rd_end, double center_x, double center_y, geometry_msgs::PoseStamped& pose_local, int i, int &fly_mode, ros::Time &last_request_local, ros::Time instant_time, double loop, bool clockwise)
{
    double x_degree, y_degree, start_dg, w, x, y, z;
    if(i == 0)
    {
        rd = sqrt(pow((pose_local.pose.position.x - center_x), 2)+pow((pose_local.pose.position.y) - center_y, 2));
    ...
    if(clockwise == false)
    {
        if(dg <=  rd_end)
        {
            pose_local.pose.position.x = center_x + rd * cos(dg);
            pose_local.pose.position.y = center_y + rd * sin(dg);
            rpy_to_Q(dg+M_PI,w,x,y,z);

            pose_local.pose.orientation.w = w;
            pose_local.pose.orientation.x = x;
            pose_local.pose.orientation.y = y;
            pose_local.pose.orientation.z = z;
            dg = dg + 0.01;
        }else
        {
            fly_mode++;
